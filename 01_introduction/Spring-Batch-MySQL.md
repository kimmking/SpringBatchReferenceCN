# 用Spring Batch从CSV文件读取数据并写入MySQL数据库 #
...
GB级别的数据量对于批处理任务来说简直就像一场海啸(tsunami),但通过Spring Batch框架你就可以将它拆解成多块(chunk)来分而治之。Spring Batch 是Spring大框架中的一个模块,是专门设计来为各种类型的文件处理批处理细节的。 本文将通过实例来学习Spring Batch,先构建一个简单的作业(job),将产品列表从CSV文件导入到MySQL数据库中; 随后我们将一起探究 Spring Batch 提供了哪些批处理功能,比如单个或多个处理单元(processors), 同时可以使用一到多个很有用的微线程(tasklets); 最后将简单介绍Spring Batch的弹性工具,包括忽略记录(skipping), 重试记录(retrying),以及重启批处理作业(restarting )。


如果你曾在Java企业系统间通过批处理来实现大批量数据的交换,那么你就能理解什么是工作负载。 你的批处理系统需要能够处理海量的数据,在处理单条记录失败时不能影响到整个过程,并能管理中断,在重新启动后也无需再次执行已处理过的部分。

对于不太了解具体情形的人来说,下面是一些需要批处理的场景,在这些情况下使用Spring Batch 可能会节省无数的时间:


- 如果接收到一个文件,但是缺少一些需要的信息时,你需要解析整个文件,调用服务来获取缺失的信息,并且写入到输出文件,给下一个批处理过程使用。
- 如果执行时发生了一个错误,则将失败信息写入数据库中。 你需要有一个进程,每15分钟一次来遍历失败信息,如果配置为可以重试,那就需要再尝试执行。
- 在工作流中,在接收到事件消息时,你希望其他系统来调用特定的服务。 如果其他系统没有调用你的服务,那么几天后需要自动清理您的数据,以使业务流程不会失败。
- 每天接收一个包含员工信息更新的文件,并需要为新员工创建工件(artifacts,账号?)。
- 有一些用于定制订单的服务。 每天晚上运行一个批处理过程,来构造清单文件并将它们发送给相应的供应商。
