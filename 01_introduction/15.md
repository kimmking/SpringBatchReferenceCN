...
这涉及到将输入的记录集合分解成偶数个部分(例如10,这样每部分是整个数据集的十分之一). 每个部分稍后由一个批处理/提取程序实例来处理.
...
为了使用这种方法,预处理需要将记录集拆分. 拆分的结果将有一个最大值和最小值位置, 这两个值可以用作限制每个 批处理/提取程序处理部分的输入.
...
预处理可能是一个巨大的开销,因为它必须计算并确定的每部分数据集的边界.
...
这涉及到将输入记录按照某个关键列来分解,比如位置代码(location code),并根据每个键来决定分配给哪个批处理实例.为了达到这个目标,列值也可以.
...
在使用第1种方法时, 新值的添加将意味着需要手工重新配置批/提取程序,以确保新值被添加到某个特定的实例.
...
在这两个选项下,并不能将指定给批处理实例的记录实现最佳均匀分布. 批处理实例的数量并不能动态配置.
...
这种方法基本上根据键列来分解,但不同的是在数据库级进行分解.它涉及到将记录集分解成视图.这些视图将被批处理程序的各个实例在处理时使用. 分解将通过数据分组来完成.
