...
Additionally a basic application shell should be provided for business logic that cannot be built using the previously mentioned building blocks.(这段可以有问题... 等待后期修正)

因为业务逻辑并不能用上面提到的这些模块来完成, 所以还需要另外提供一个基础程序外壳(shell),。

In addition to the main building blocks, each application may use one or more of standard utility steps, such as:

除了主要的这些模块,每个应用还可以使用一个或多个标准的实用程序的步骤,如:
